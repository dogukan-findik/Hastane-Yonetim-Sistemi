...
function NewAppointmentForm({ onSubmit }) {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/doctors/listele')
      .then(res => {
        setDoctors(res.data);
        // Bölümleri doktorlardan türet
        const bolumler = [...new Set(res.data.map(d => d.UzmanlikAlani))];
        setDepartments(bolumler);
      });
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    // Tarihi normalize et (sadece gün)
    const dateOnly = new Date(selectedDate);
    dateOnly.setHours(0, 0, 0, 0);

    await axios.post('http://localhost:5000/api/randevu/ekle', {
      Bolum: selectedDepartment,
      DoktorID: selectedDoctor,
      DoktorAdSoyad: selectedDoctorObj.Ad + ' ' + selectedDoctorObj.Soyad,
      HastaID: userInfo._id,
      HastaAdSoyad: userInfo.Ad + ' ' + userInfo.Soyad,
      Tarih: dateOnly,
      Saat: selectedTime,
      Durum: 'Beklemede'
    });
  ...
  const filteredDoctors = doctors.filter(d => d.UzmanlikAlani === selectedDepartment);
